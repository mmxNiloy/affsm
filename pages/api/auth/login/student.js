...
const handler = async (req, res) => {
    ...
    const encodedID = encodeMessage(user.user_id + '') + encodeMessage((Date.now() % 1000000).toString().padStart(8, '0'))

    cookies.set(process.env.MY_SECRET_USER_KEY + encodedID, token)

    console.log('Login successful')

    return res.status(200).json({message: 'Authentication successful!', secret: encodedID})
}
